...
class RTFNet(nn.Module):
    ...
    def __init__(self, n_class, layers = 50):
        super(RTFNet, self).__init__()

        self.num_resnet_layers = layers

        if self.num_resnet_layers == 18:
            resnet_raw_model1 = models.resnet18(pretrained=False)
            resnet_raw_model2 = models.resnet18(pretrained=False)
            self.inplanes = 512
        elif self.num_resnet_layers == 34:
            resnet_raw_model1 = models.resnet34(pretrained=False)
            resnet_raw_model2 = models.resnet34(pretrained=False)
            self.inplanes = 512
        elif self.num_resnet_layers == 50:
            resnet_raw_model1 = models.resnet50(pretrained=False)
            resnet_raw_model2 = models.resnet50(pretrained=False)
            self.inplanes = 2048
        elif self.num_resnet_layers == 101:
            resnet_raw_model1 = models.resnet101(pretrained=False)
            resnet_raw_model2 = models.resnet101(pretrained=False)
            self.inplanes = 2048
        elif self.num_resnet_layers == 152:
            resnet_raw_model1 = models.resnet152()
            resnet_raw_model2 = models.resnet152()
            self.inplanes = 2048
...
if __name__ == '__main__':
    ...
    rtf_net = RTFNet(9,50).cuda(0)
    input = torch.cat((rgb, thermal), dim=1)
    flops, params = profile(rtf_net, inputs=(input,))
    params = params / 1e6
    flops = flops / 1e9
    print("params: %.2fM, flops: %.2fG" % (params, flops))
    for i in range(10):
        rtf_net(input)
        # GPU memory usage
