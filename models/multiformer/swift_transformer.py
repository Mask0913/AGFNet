...
def get_model(model_name, modal):
    if "XS" in model_name:
        model = SwiftFormer(
            layers=SwiftFormer_depth['XS'],
            embed_dims=SwiftFormer_width['XS'],
            downsamples=[True, True, True, True])
        pre_train = torch.load("model_pth/swift_transformer/SwiftFormer_XS.pth")
        stem_dem = SwiftFormer_width['XS'][0]
    elif "S" in model_name:
        model = SwiftFormer(
            layers=SwiftFormer_depth['S'],
            embed_dims=SwiftFormer_width['S'],
            downsamples=[True, True, True, True])
        pre_train = torch.load("model_pth/swift_transformer/SwiftFormer_S.pth")
        stem_dem = SwiftFormer_width['S'][0]
    elif "l1" in model_name:
        model = SwiftFormer(
            layers=SwiftFormer_depth['l1'],
            embed_dims=SwiftFormer_width['l1'],
            downsamples=[True, True, True, True])
        pre_train = torch.load("model_pth/swift_transformer/SwiftFormer_l1.pth")
        stem_dem = SwiftFormer_width['l1'][0]
    elif "l3" in model_name:
        model = SwiftFormer(
            layers=SwiftFormer_depth['l3'],
            embed_dims=SwiftFormer_width['l3'],
            downsamples=[True, True, True, True])
        pre_train = torch.load("model_pth/swift_transformer/SwiftFormer_l3.pth")
        stem_dem = SwiftFormer_width['l3'][0]
    else:
        raise ValueError("model_name error!")
    if modal == "RGB":
        model.load_state_dict(pre_train["model"], strict=False)
        return model
    else:
        model.load_state_dict(pre_train["model"], strict=False)
        model.patch_embed = stem(1, stem_dem)
        return model
...
class SwiftFormer(nn.Module):
    ...
    def get_normal(self):
        return nn.ModuleList([self.norm0, self.norm2, self.norm4, self.norm6])
